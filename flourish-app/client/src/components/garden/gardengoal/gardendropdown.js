...
export default function GardenDropDown({optionSetter, options}){
    ...
    return(
        <div>
            <button className="bg-white border-jet border-[2px] w-[250px] h-[2em] text-left pl-[10px]"
            onClick={function(){
                setOpen(!open);
            }}>
            ...
            {options.map(function(opt, key){
                return(
                    <div>
                        <button className="fixed bg-white border-jetbright border-[1px] w-[250px] h-[1.75em] text-left pl-[10px]"
                        style={{marginTop:String(1.75*key)+"em"}}
                        onClick={function(){
                            optionSetter(opt);
                            setSelect(opt);
                            setOpen(!open);
